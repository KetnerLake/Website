...
export default async ( request, context ) => {
  try {
    ...
    if( body.length > 1 ) {
      body[body.length - 1] = 'and ' + body[body.length - 1];
    }
    ...
    const completion = await openai.chat.completions.parse( {
      ...
      messages: [
        ...
        {role: 'user', content: `I enjoy the following cigar${body.length > 1 ? 's' : ''}: ${body.join( ', ' )}. Recommend five other similar cigars.`}
      ],
